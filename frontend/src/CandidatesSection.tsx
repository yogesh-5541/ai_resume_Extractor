...
export default function CandidatesSection() {
  ...
  return (
    <div className="glass p-8 rounded-3xl border border-white/[0.08] mt-12 transition-all relative">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 pb-4 border-b border-slate-100">
        <div>
          <h2 className="text-2xl font-extrabold text-slate-100 flex items-center tracking-tight">
            All Candidates
          </h2>
          <p className="text-slate-500 text-sm mt-1">Review all uploaded and processed resumes in your system.</p>
        </div>
        
        <div className="mt-4 md:mt-0 flex items-center bg-white/[0.04] p-1.5 rounded-xl border border-white/[0.06]">
           <button 
             onClick={() => setFilterSuccessOnly(false)}
             className={`px-5 py-2 rounded-lg text-sm font-semibold transition-all duration-300 ${!filterSuccessOnly ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-500/25 scale-105' : 'text-slate-400 hover:text-white'}`}>
            All List
          </button>
          <button 
             onClick={() => setFilterSuccessOnly(true)}
             className={`px-5 py-2 rounded-lg text-sm font-semibold transition-all duration-300 ${filterSuccessOnly ? 'bg-emerald-500/20 text-emerald-300 shadow-md border border-emerald-500/30 scale-105' : 'text-slate-400 hover:text-white'}`}>
            Success Only
          </button>
        </div>
        ...
        <div className="mt-4 md:mt-0 xl:absolute xl:right-8 xl:top-8">
          <button 
            ...
            className="px-4 py-2 bg-red-500/10 text-red-400 hover:bg-red-500/20 border border-red-500/20 rounded-xl text-sm font-semibold transition-colors flex items-center shadow-sm"
          >
      ...
      {isLoading ? (
        ...
        <div className="text-center py-20 bg-white/[0.02] rounded-2xl border border-dashed border-white/[0.1]">
          <p className="text-slate-400 font-medium">No candidates match your current filter.</p>
        </div>
      ) : (
          ...
          {filteredCandidates.map(candidate => {
            ...
            return (
              <div 
                ...
                className={`group glass rounded-3xl border border-white/[0.08] transition-all duration-500 overflow-hidden ring-1 ring-slate-900/5 
                ${isExpanded ? 'shadow-2xl shadow-indigo-500/10 scale-[1.01] border-indigo-500/20 xl:col-span-3 md:col-span-2 cursor-default' : 'shadow-lg shadow-black/30 hover:shadow-xl hover:-translate-y-1 hover:border-white/[0.15] cursor-pointer'}`}
              >
                ...
                <div className="p-6 relative">
                  ...
                  <div className="flex items-center space-x-5">
                    <div className="w-14 h-14 rounded-full bg-gradient-to-br from-indigo-500/30 to-purple-500/30 flex items-center justify-center text-indigo-300 font-bold text-xl border border-indigo-500/20">
                      {name !== 'Unknown' ? name.charAt(0).toUpperCase() : '?'}
                    </div>
                    <div className="flex-1 min-w-0 pr-20">
                      <h3 className={`text-xl font-bold truncate tracking-tight transition-colors ${isExpanded ? 'text-indigo-400' : 'text-slate-100 group-hover:text-indigo-400'}`}>{name}</h3>
                      <p className="text-sm font-semibold text-indigo-400 truncate mt-0.5">{candidate.currentJobTitle || 'Candidate'}</p>
                      <p className="text-xs font-medium text-slate-500 truncate mt-0.5">{email}</p>
                    </div>
                  </div>
                  
                  <div className="mt-6 flex flex-wrap items-center justify-between gap-4">
                     <div className="inline-flex items-center bg-white/[0.05] px-3 py-1.5 rounded-lg border border-white/[0.08] text-sm">
                        ...
                        <span className="font-bold text-slate-200">{exprYears}</span>
                        <span className="text-slate-400 ml-1 font-medium">Years</span>
                     </div>

                     <div className="flex items-center space-x-2">
                       {hasSkills ? topSkills.map((skill, idx) => (
                          <span key={idx} className="px-3 py-1 bg-white/[0.06] text-slate-300 text-[11px] font-bold uppercase tracking-wider rounded-md border border-white/[0.08]">
                            {skill}
                          </span>
                       )) : (
                       ...
                       {!isExpanded && hiddenSkillsCount > 0 && (
                          <span className="px-2 py-1 bg-indigo-500/15 text-indigo-400 text-[11px] font-bold rounded-md border border-indigo-500/20">
                             +{hiddenSkillsCount}
                          </span>
                       )}
                     </div>
                    ...
                    <div className="p-6 bg-white/[0.02]">
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                            <div className="space-y-6">
                                <div>
                                    <h4 className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-3 flex items-center"><svg className="w-4 h-4 mr-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"></path></svg> Phone Details</h4>
                                    <p className="text-sm font-semibold text-slate-200 bg-white/[0.05] inline-flex px-4 py-2 rounded-lg border border-white/[0.08]">{phone}</p>
                                </div>
                                
                                {candidate.education && (
                                  <div>
                                    <h4 className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-3 flex items-center"><svg className="w-4 h-4 mr-1.5 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 14l9-5-9-5-9 5 9 5z"></path><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 14l6.16-3.422a12.083 12.083 0 01.665 6.479A11.952 11.952 0 0012 20.055a11.952 11.952 0 00-6.824-2.998 12.078 12.078 0 01.665-6.479L12 14z"></path></svg> Education</h4>
                                    <p className="text-sm font-semibold text-slate-200 bg-white/[0.05] inline-flex px-4 py-2 rounded-lg border border-white/[0.08]">{candidate.education}</p>
                                  </div>
                                )}
                                ...
                                <div>
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {hasSkills ? rawSkills.map((skill, idx) => (
                                           <span key={`expanded-${idx}`} className="px-3 py-1.5 bg-white/[0.05] text-slate-300 text-[11px] font-bold uppercase tracking-wider rounded-md border border-white/[0.08] hover:-translate-y-0.5 transition-all cursor-default">
                                             {skill}
                                           </span>
                                        )) : (
                            ...
                            <div className="flex flex-col gap-6">
                              ...
                              <div className="bg-white/[0.03] rounded-2xl p-5 border border-white/[0.06] flex-1">
                                  ...
                                  <div className="h-40 overflow-y-auto pr-3 text-xs text-slate-500 font-mono leading-relaxed">
                ...
                <div className={`bg-white/[0.02] py-3 border-t border-white/[0.06] transition-colors ${isExpanded ? '' : 'group-hover:bg-indigo-500/5 cursor-pointer'}`}>
                    {isExpanded ? (
                        <div className="flex items-center justify-between px-6">
                            <span className="text-[11px] font-bold text-slate-500 uppercase tracking-widest flex items-center">
                        ...
                        <span className="text-[11px] font-bold text-slate-500 uppercase tracking-widest group-hover:text-indigo-400 transition-colors flex items-center justify-center cursor-pointer">
                            View Full Details
