...
export default function Dashboard() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 animate-fade-in-up">
      ...
      <div className="flex items-center gap-2 mb-8 p-1.5 bg-slate-100/70 rounded-2xl border border-slate-200/70 w-fit shadow-inner">
        {(['ranking', 'candidates', 'analytics'] as const).map(tab => (
          ...
          >
            {tab === 'ranking' ? '🎯 Job Ranking' : tab === 'candidates' ? '👥 All Candidates' : '📊 Analytics'}
          </button>
        ))}
      </div>
            ...
            <div className="bg-white p-6 rounded-2xl shadow-md border border-slate-100 transition hover:shadow-lg">
              ...
              <form onSubmit={handleJobSubmit} className="space-y-4">
                <div>
                  ...
                  <input required type="text" className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none transition" placeholder="e.g. Senior Frontend Engineer" value={jobForm.title} onChange={e => setJobForm({...jobForm, title: e.target.value})} />
                </div>
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">Required Skills</label>
                  <input required type="text" className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none transition" placeholder="e.g. React, TypeScript, Tailwind" value={jobForm.requiredSkills} onChange={e => setJobForm({...jobForm, requiredSkills: e.target.value})} />
                </div>
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">Min Experience (Years)</label>
                  <input required type="number" step="0.5" className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none transition" value={jobForm.minExperience} onChange={e => setJobForm({...jobForm, minExperience: parseFloat(e.target.value)})} />
                </div>
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">Description</label>
                  <textarea className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none transition min-h-[100px]" placeholder="Briefly describe the role..." value={jobForm.description} onChange={e => setJobForm({...jobForm, description: e.target.value})}></textarea>
                </div>
                {jobError && <div className="p-3 bg-red-50 text-red-600 text-sm rounded-lg font-medium border border-red-100">{jobError}</div>}
                <button type="submit" disabled={isSubmittingJob} className={`w-full py-3 rounded-lg text-white font-semibold transition shadow-md hover:shadow-lg flex justify-center items-center ${isSubmittingJob ? 'bg-indigo-400 cursor-wait' : 'bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700'}`}>
                  {isSubmittingJob ? <svg className="animate-spin h-5 w-5 text-white" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg> : 'Create & Rank Candidates'}
                </button>
              </form>
            </div>
                ...
                <div className="space-y-6">
                  {rankedCandidates.map((candidate, idx) => (
                    <div key={candidate.resumeId} className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm hover:shadow-md transition-all duration-300 relative overflow-hidden group">
                      {idx === 0 && (
                        <div className="absolute top-0 right-0 bg-yellow-400 text-yellow-900 text-xs font-bold px-3 py-1 rounded-bl-lg shadow-sm">Top Match</div>
                      )}
                          ...
                          <div className="flex flex-wrap gap-2">
                            {candidate.matchedSkills && candidate.matchedSkills.length > 0 ? (
                              candidate.matchedSkills.map(skill => (
                                <span key={skill} className="px-2.5 py-1 bg-green-100 text-green-700 text-xs font-semibold rounded-md border border-green-200 shadow-sm">{skill}</span>
                              ))
                          ...
                          <div className="flex flex-wrap gap-2">
                            {candidate.missingSkills && candidate.missingSkills.length > 0 ? (
                              candidate.missingSkills.map(skill => (
                                <span key={skill} className="px-2.5 py-1 bg-red-100 text-red-700 text-xs font-semibold rounded-md border border-red-200 shadow-sm">{skill}</span>
                              ))
                            ) : <span className="text-sm text-slate-400">None</span>}
                          </div>
                        </div>
